...
BEGIN
    ...
    IF v_saldo_atual < v_valor THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'ERRO: Saldo insuficiente';
    END IF;
...
BEGIN
    ...
    -- procura o valor bruto calculado da view
    ...
    /*  atualiza a folha para aprovado ANTES de criar o lançamento
    garante que o trigger de update seja disparado com as validações */
    ...
    -- atualiza o id do lancamento na folha
    UPDATE folha_funcionarios
...
BEGIN
    ...
    -- mesmo de antes para o elenco
    UPDATE folha_elenco
    SET status = 'aprovado'
    WHERE id_folha_elenco = p_id_folha;
    
    -- cria o lançamento pós aprovação
    INSERT INTO lancamento (
        data_registro,
        valor, 
        tipo_de_movimentacao, 
...
BEGIN
    ...
    -- deixa  a folha do elenco como pendente
...
BEGIN
    ...
    -- insere na folha somente como pendente para aprovação
    ...
    -- chama a aprovação da folha dos funcionarios
